...
// Name change of Dashboard to My Courses
...
observer.observe(document.body, { childList: true, subtree: true });
    ...
    /**
     * Fetches all modules (with items + content_details) for a course.
     * Returns the parsed modules array, or null on failure.
     */
    async fetchModulesWithItems(courseId) {
        try {
    ...
    hasWebinarContext() {
        ...
        // ----------------------------
        if (!window.location.hash.includes("view_name=agenda")) return false;

        const courseId = this.getCourseId({ fromContext: true });
        return courseId
            ? this.webinarCalendarContexts.includes(`course_${courseId}`)
            : false;
    }
    ...
    async applyStudentStyles() {
        ...
        const alwaysHiddenCSS = this.alwaysHiddenElements
            .map(id => `#${id} { display: none !important; }`)
            .join('\n');

        // Conditionally hide right-side-wrapper
        let conditionalCSS = '';

        const shouldShowRightSideWrapper = this.showRightSideWrapperOn
            .some(path => currentPath.startsWith(path));

        if (!shouldShowRightSideWrapper) {
            conditionalCSS = this.conditionallyHiddenElements
                .map(id => `#${id} { display: none !important; }`)
                .join('\n');
        }
        ...
        styleEl.id = styleId;
        styleEl.textContent = alwaysHiddenCSS + '\n' + conditionalCSS;
    ...
    queueWebinarAutoSelect() {
        ...
        this.waitFor(
            () =>
                ...
                document.querySelector('.agenda-event__item-container'),
            () => {
    ...
    createWebinarReturnButton() {
        const parent = document.getElementById("other-right-side-wrapper") || document.createElement("div");
        ...
        btn.className = "css-1sec0gm-view--inlineBlock-baseButton";
        btn.setAttribute("role", "button");

        const contentSpan = document.createElement("span");
        contentSpan.className = "css-1ta5ds2-baseButton__content";
        ...
        childrenSpan.appendChild(flexSpan);
        contentSpan.appendChild(childrenSpan);
        ...
        console.log("✅ Webinar return button created (class)", courseId);
    }
    ...
    async createHomeButtons() {
        try {
            ...
            // Fetch modules + items
            const modules = await this.fetchModulesWithItems(courseId);
            if (!modules) return;
